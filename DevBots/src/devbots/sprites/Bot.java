...
import static devbots.Global.MAX_AP;
...
import static devbots.Global.LOOK_AP;
import static devbots.Global.LOOK_FUEL;
import static devbots.Global.SCAN_AP;
...
import devbots.actions.BotAction;
 ...
 * @author Raymond
 */
public class Bot extends SyncSprite
{

    public static final ArrayList<BotAction> ACTIONS = new ArrayList<>();
        ...
        if (spend(LOOK_AP, LOOK_FUEL))
        {
            ...
            for (int v = 1; v <= VISION_RANGE; v++)
            {
                ...
                if (MAP[p.x][p.y] != null)
                {
                    ...
                    {
                        this.sightObject = ((SyncSprite) obj).getVisionCode();
            ...
            for (int d = 0; d < AbsDir.values().length; d++)
            {
                ...
                } else
                {
                    if (MAP[x][y] != null)
                    {
                        ...
                        if (obj instanceof Sprite)
                        {
                            scanObjects[c + sd.getX()][c + sd.getY()] = ((SyncSprite) obj).getVisionCode();
                        }
                    } else
                    {
    ...
    {
        if ((!isBlocked(d)) && spend(MOVE_AP, MOVE_FUEL))
        {
            ACTIONS.add(new BotMove(this, d));
            return true;
        } else
        {
            return false;
    ...
    {
        if (spend(TURN_AP, TURN_FUEL))
        {
            ACTIONS.add(new BotTurn(this, d));
            return true;
        } else
        {
            return false;
            ...
            for (int v = 1; v <= VISION_RANGE; v++)
            {
                ...
                if (MAP[p.x][p.y] != null)
                {
                    ...
                    if (obj instanceof Sprite)
                    {
                        ...
                        ACTIONS.add(new BotLaser(this, target, targetPt));
                        
                        // Remove after testing:
                        new Explosion().bindTo(target);
